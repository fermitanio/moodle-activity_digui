...
//digui_span_delete_by_version_greater_than($subdiguiid, $pageversion);
//digui_version_reset_by_subdigui($subdiguiid, $pagenum);
...
// Show the span tags for the selected user. The span tags of the selected user
// are not modified, unless the selected user is the current user.
$pageversion = digui_version_get_current($diguiid, $selectedsubdiguiid, $pagenum);
$spans = digui_span_get_by_subdiguiid($diguiid, $selectedsubdiguiid, $pagenum, $pageversion);
// Insert all spans into text.
...
echo '<div>';
digui_print_view_links('viewi', $digui->numpags, $pagenum, $diguiid, $subdiguiid, $selectedsubdiguiid, $groupid, $userid);
echo '</div>';
...
echo '<form id="formdata" method="post" action="viewi.php?diguiid='.$diguiid.'&subdiguiid='.$subdiguiid.'&group='.$groupid.'&uid='.$userid.'&pagenum='.$pagenum.'">
