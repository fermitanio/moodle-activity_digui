...
// Restrict editing the current page, if a user is currently editing that page.
$restrictediting = false;
$record = $DB->get_record('digui_last_user_modification', array('diguiid' => $diguiid, 'pagenum' => $pagenum));
$currenttime = time();
if ($record && $record->subdiguiid != $subdiguiid && $currenttime - $record->timemodified < 300) {
    $restrictediting = true;
}
else {
    digui_version_save_last_time_modification($diguiid, $pagenum, $subdiguiid, time());
}
...
if (!$restrictediting && !empty($SESSION->has_timed_out) && $SESSION->has_timed_out == true) {
...
// saved of the spans.
if (!$restrictediting && isset($_SERVER['HTTP_REFERER'])) {    
    $s = $_SERVER['HTTP_REFERER'];
    if (!preg_match('/(.*)editc.php(.*)/s', $_SERVER['HTTP_REFERER'])) { 
        digui_span_delete_by_version_greater_than($diguiid, NULL, $pagenum, $pageversion);
        digui_span_delete_by_version_less_than($diguiid, NULL, $pagenum, $pageversion);
        digui_version_reset($diguiid, NULL, $pagenum);
        $pageversion = 1;
...
digui_version_save_number($diguiid, NULL, $pagenum, $pageversion);
...
if (!$restrictediting) {
    ...
    echo '</div>';
    echo '<div style="background-color:#C0C0C0; overflow: scroll; border-style: solid; border-color: #E0E0E0 ; border-width: 1px; width: 100%; margin: 0 auto; padding: 5px; text-align: justify; font-size:14px;">'.get_string('restrictediting', 'digui').'</div>';
}
...
for ($i = 0, $j = strlen($page->cachedcontent); $i < $j; $i++) {
    if ($page->cachedcontent[$i] == "<") {
        $ltcharacters .= $i;
        $ltcharacters .= ",";
    }
    else if ($page->cachedcontent[$i] == ">") {
        $gtcharacters .= $i;
        $gtcharacters .= ",";
    }
}

// Delete the last character (a comma), in gtcharacters and ltcharacters 
// variables.
if (strlen($gtcharacters) > 0) {
    $gtcharacters = substr($gtcharacters, 0, strlen($gtcharacters) - 1);
}
if (strlen($ltcharacters) > 0) {
    $ltcharacters = substr($ltcharacters, 0, strlen($ltcharacters) - 1);
}

$wholetext = $page->cachedcontent;
// will be part of ...
$wholetext = str_replace('"','\'', $wholetext);

// Replace opening and closing angles, by underline characters.
if (strlen($ltcharacters) > 0) {
    $ltcharactersarray = explode(',', $ltcharacters); 
    for ($i = 0, $j = count($ltcharactersarray); $i < $j; $i++) {
        $k = $ltcharactersarray[$i];
        $k = intval($k);
        $wholetext = substr($wholetext, 0, $k) . "_" . substr($wholetext, $k+1);
    }

    $gtcharactersarray = explode(',', $gtcharacters); 
    for ($i = 0, $j = count($gtcharactersarray); $i < $j; $i++) {
        $k = $gtcharactersarray[$i];
        $k = intval($k);
        $wholetext = substr($wholetext, 0, $k) . "_" . substr($wholetext, $k+1);
    }
}
...
echo '<form id="hform" method="post" onsubmit="sendSubrayado()" action="editc.php?diguiid='.$diguiid.'&subdiguiid='.$subdiguiid.'&group='.$groupid.'&uid='.$userid.'&pagenum='.$pagenum.'&action=highlight">
...
if (!$restrictediting) {
    ...
    echo '<form id="undoform" method="post" style="display:inline;" action="editc.php?diguiid='.$diguiid.'&subdiguiid='.$subdiguiid.'&group='.$groupid.'&uid='.$userid.'&pagenum='.$pagenum.'&action=undo">
    ...
    echo '<form id="redoform" method="post" style="display:inline;" action="editc.php?diguiid='.$diguiid.'&subdiguiid='.$subdiguiid.'&group='.$groupid.'&uid='.$userid.'&pagenum='.$pagenum.'&action=redo">
...
echo '<form id="sform" method="post" onsubmit="sendNotes()" action="editc.php?diguiid='.$diguiid.'&subdiguiid='.$subdiguiid.'&group='.$groupid.'&uid='.$userid.'&pagenum='.$pagenum.'&action=save">
